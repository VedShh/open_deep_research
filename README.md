# The Open Deep Research Project
The "Open Deep Research" project is a Retrieval-Augmented Generation (RAG) system designed to synthesize information from multiple research papers and generate a coherent, high-quality summary or introduction on a given topic. The project aims to automate a time-consuming and labor-intensive task that is typically performed manually by researchers and PhD students. The ultimate goal is to create an AI/ML model that can efficiently analyze and summarize research, with a specific focus on the domain of Mars seismology. The user's notes emphasize the desire to build a program specifically for academic use.

# The Problem This Project Addresses
The core problem this project seeks to solve is the inefficiency and time commitment involved in manual research synthesis. For a researcher or a PhD student, it can take up to a year to understand all the existing research and papers on a single topic. The process of reading dozens of papers, extracting key information, and then synthesizing that information into a new document, such as an introduction or a literature review, is a significant bottleneck. The project's proposed solution is to use an AI agent to drastically reduce this time by automating the process of deep research and paper summarization.

# My Work and Solutions
My work on this project is focused on building and refining a multi-agent RAG system using the LangChain and LangGraph frameworks. The solution involves several key steps and a continuous feedback loop for improvement.
Data Preparation: The first step is to process raw research papers. The agent must be able to scan papers and extract only the essential sections, such as the introduction, abstract, methods, results, discussion, and conclusion, while removing unnecessary information like key references. The notes mention using an LLM as a "last effort" to parse and clean up a PDF, if needed.
- Vector Database: The extracted information from the papers is then stored in a vector database. The user's notes mention the need to grade each paper in the database based on its relevance and quality to ensure the generated introduction is of high quality. A potential future direction for scaling is to move beyond cosine similarity for more efficient searches.
- Agentic Architecture: The project uses a multi-agent system, similar to the one described by Anthropic, with a "lead researcher" agent that assesses a task and delegates smaller pieces of work to sub-agents. These sub-agents can run multiple tools simultaneously, such as searching the web or a local vector database, before returning their findings to the lead agent.
- Prompt Engineering and Evaluation: A significant portion of the work involves meticulous prompt engineering. The user's notes highlight that prompt engineering is an iterative process. The strategy is to have a model (like GPT or Gemini) iterate on the initial prompt, and then further refine it by having the model analyze a well-written, industry-standard introduction to learn how to produce a better output. The ultimate test for success is if the agent-generated introduction can be deemed "better" than a human-written one based on a pre-defined set of criteria, and then validated through user testing. This is done using a loop of creating, improving the database, improving the prompt, and testing again.
- Challenges and Validation: I have encountered and worked through various technical challenges, as shown in the provided images of TypeError and FileNotFoundError. The notes also emphasize the importance of using tests to validate each step of the process. The project is also exploring the use of a Model Context Protocol (MCP) as it is becoming a standard in the AI community and allows for the reuse of existing open-source code.
In essence, my solution is not just a single program but an iterative development process that focuses on building a robust, multi-agent system capable of complex research synthesis.
...
# <----------------------------------------------------- End Personal README --- Starts Langchain Original README ------------------------------------------------------------------->
